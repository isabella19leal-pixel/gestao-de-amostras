...
$("fOS").addEventListener("input", () => { currentPage = 1; aplicarFiltros(); });
...
function aplicarFiltros() {
  const osFiltro = $("fOS").value.trim().toLowerCase();
  ...
  filteredAmostras = allAmostras.filter((a) => {
    if (osFiltro && !a.ordem_servico.toLowerCase().includes(osFiltro)) return false;
    if (busca) {
      const alvo = `${a.codigo_amostra} ${a.cliente || ""}`.toLowerCase();
...
function renderTabela() {
  ...
  $("tableBody").innerHTML = pageItems.map((a) => `
    <tr>
      <td class="code">${a.ordem_servico}</td>
